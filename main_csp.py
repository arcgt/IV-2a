...
from ranking import dimension_reduction, ranking, channel_selection
...
class CSP_Model:
	def __init__(self):
		...
		self.forder= 2 # 4
		self.filter_bank = load_filterbank(self.bw,self.fs,order=self.forder,max_freq=32,ftype = self.ftype) # get filterbank coeffs
...
def main():
	...
	w_avg = dimension_reduction(w_avg_4d, model.NO_channels, model.NO_csp)
	print(w_avg)
